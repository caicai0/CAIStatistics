...
+ (instancetype)keyPointForString:(NSString *)string{
    if (string && [string isKindOfClass:[NSString class]] && string.length) {
        ...
        if (!hasError) {
            CAISDSKeyPoint * keypath = [[CAISDSKeyPoint alloc]init];
...
- (NSString *)stringValueForInfo:(id<CAISDSAspectInfo>)info{
    NSString * result = @"NULL";
    @try {
        if (self.type == CAISKeyPointTypeKeyPathString) {
            if (info && [info conformsToProtocol:@protocol(CAISDSAspectInfo)]) {
            ...
            id object = [CAISDSUtils objectForClass:self.aclass selector:self.selector keyPath:self.keyPath];
            NSString * description = [object description];
            if (description) {
                result = description;
            }else{
