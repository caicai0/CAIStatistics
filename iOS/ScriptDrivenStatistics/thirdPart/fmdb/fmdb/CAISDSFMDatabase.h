...
/** A SQLite ([http://sqlite.org/](http://sqlite.org/)) Objective-C wrapper.
 ...
 - `CAISDSFMDatabase` - Represents a single SQLite database.  Used for executing SQL statements.
 - `<CAISDSFMResultSet>` - Represents the results of executing a query on an `CAISDSFMDatabase`.
 - `<CAISDSFMDatabaseQueue>` - If you want to perform queries and updates on multiple threads, you'll want to use this class.
 ...
 @warning Do not instantiate a single `CAISDSFMDatabase` object and use it across multiple threads. Instead, use `<CAISDSFMDatabaseQueue>`.
...
@interface CAISDSFMDatabase : NSObject
...
/** Create a `CAISDSFMDatabase` object.
 
 An `CAISDSFMDatabase` is created with a path to a SQLite database file.  This path can be one of these three:

 1. A file system path.  The file does not have to exist on disk.  If it does not exist, it is created for you.
 2. An empty string (`@""`).  An empty database is created at a temporary location.  This database is deleted with the `CAISDSFMDatabase` connection is closed.
 3. `nil`.  An in-memory database is created.  This database will be destroyed with the `CAISDSFMDatabase` connection is closed.

 For example, to create/open a database in your Mac OS X `tmp` folder:

    CAISDSFMDatabase *db = [CAISDSFMDatabase databaseWithPath:@"/tmp/tmp.db"];

 Or, in iOS, you might open a database in the app's `Documents` directory:

    NSString *docsPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
    NSString *dbPath   = [docsPath stringByAppendingPathComponent:@"test.db"];
    CAISDSFMDatabase *db     = [CAISDSFMDatabase databaseWithPath:dbPath];
 ...
 @return `CAISDSFMDatabase` object if successful; `nil` if failure.
...
/** Create a `CAISDSFMDatabase` object.
 ...
 2. `nil`.  An in-memory database is created.  This database will be destroyed with the `CAISDSFMDatabase` connection is closed.
...
/** Initialize a `CAISDSFMDatabase` object.
...
/** Initialize a `CAISDSFMDatabase` object.
 ...
 An `CAISDSFMDatabase` is created with a local file URL to a SQLite database file.  This path can be one of these three:
...
/** Execute select statement
 ...
 Executing queries returns an `<CAISDSFMResultSet>` object if successful, and `nil` upon failure.  Like executing updates, there is a variant that accepts an `NSError **` parameter.  Otherwise you should use the `<lastErrorMessage>` and `<lastErrorMessage>` methods to determine why a query failed.
 
 In order to iterate through the results of your query, you use a `while()` loop.  You also need to "step" (via `<[CAISDSFMResultSet next]>`) from one record to the other.
 ...
 @return A `<CAISDSFMResultSet>` for the result set upon success; `nil` upon failure. If failed, you can call `<lastError>`, `<lastErrorCode>`, or `<lastErrorMessage>` for diagnostic information regarding the failure.
 
 @see CAISDSFMResultSet
 @see [`CAISDSFMResultSet next`](<[CAISDSFMResultSet next]>)
 @see [`sqlite3_bind`](http://sqlite.org/c3ref/bind_blob.html)
...
- (CAISDSFMResultSet * _Nullable)executeQuery:(NSString*)sql, ...;
...
- (CAISDSFMResultSet * _Nullable)executeQueryWithFormat:(NSString*)format, ... NS_FORMAT_FUNCTION(1,2);
...
- (CAISDSFMResultSet * _Nullable)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments;
...
/** Execute select statement
 ...
 `func executeQuery(sql: String, values: [Any]?) throws  -> CAISDSFMResultSet!`
...
- (CAISDSFMResultSet * _Nullable)executeQuery:(NSString *)sql values:(NSArray * _Nullable)values error:(NSError * _Nullable __autoreleasing *)error;
...
- (CAISDSFMResultSet * _Nullable)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary * _Nullable)arguments;


// Documentation forthcoming.
- (CAISDSFMResultSet * _Nullable)executeQuery:(NSString *)sql withVAList:(va_list)args;
...
/** Adds SQL functions or aggregates or to redefine the behavior of existing SQL functions or aggregates.
 ...
 For example:
    ...
    CAISDSFMResultSet *rs = [db executeQuery:@"SELECT * FROM employees WHERE RemoveDiacritics(first_name) LIKE 'jose'"];
...
/** Generate an `NSDateFormatter` that won't be broken by permutations of timezones or locales.
 ...
 Example:
    ...
    myDB.dateFormat = [CAISDSFMDatabase storeableDateFormat:@"yyyy-MM-dd HH:mm:ss"];
...
/** Set to a date formatter to use string dates with sqlite instead of the default UNIX timestamps.
 ...
 @param format Set to nil to use UNIX timestamps. Defaults to nil. Should be set using a formatter generated using CAISDSFMDatabase::storeableDateFormat.
