...
/** Represents the results of executing a query on an `<SRPFMDatabase>`.
...
@interface SRPFMResultSet : NSObject

@property (nonatomic, retain, nullable) SRPFMDatabase *parentDB;
...
/** Create result set from `<FMStatement>`
 ...
 @param aDB A `<SRPFMDatabase>` to be used
 
 @return A `SRPFMResultSet` on success; `nil` on failure
 */
