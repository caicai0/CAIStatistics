...
- (void)pushDatabaseBackInPool:(SRPFMDatabase*)db;
- (SRPFMDatabase*)db;

@end


@implementation SRPFMDatabasePool
...
- (void)pushDatabaseBackInPool:(SRPFMDatabase*)db {
    ...
    [self executeLocked:^() {
        ...
        if ([self->_databaseInPool containsObject:db]) {
            [[NSException exceptionWithName:@"Database already in pool" reason:@"The SRPFMDatabase being put back into the pool is already present in the pool" userInfo:nil] raise];
        }
