...
/*
 ...
 SRPFMDatabaseQueue is released on another thread and we're in the middle of doing
 something in dispatch_sync
 
 */

/*
 * A key used to associate the SRPFMDatabaseQueue object with the dispatch_queue_t it uses.
 * This in turn is used for deadlock detection by seeing if inDatabase: is called on
 * the queue's dispatch queue, which should not happen and causes a deadlock.
 */
static const void * const kDispatchQueueSpecificKey = &kDispatchQueueSpecificKey;

@interface SRPFMDatabaseQueue () {
    dispatch_queue_t    _queue;
    SRPFMDatabase          *_db;
}
@end

@implementation SRPFMDatabaseQueue

+ (instancetype)databaseQueueWithPath:(NSString *)aPath {
    SRPFMDatabaseQueue *q = [[self alloc] initWithPath:aPath];
...
+ (instancetype)databaseQueueWithPath:(NSString *)aPath flags:(int)openFlags {
    SRPFMDatabaseQueue *q = [[self alloc] initWithPath:aPath flags:openFlags];
...
#endif
        if (!success) {
            NSLog(@"SRPFMDatabaseQueue could not reopen database for path %@", _path);
            FMDBRelease(_db);
            _db  = 0x00;
            return 0x00;
        }
...
- (void)inDatabase:(void (^)(SRPFMDatabase *db))block {
#ifndef NDEBUG
    /* Get the currently executing queue (which should probably be nil, but in theory could be another DB queue
     * and then check it against self to make sure we're not about to deadlock. */
    SRPFMDatabaseQueue *currentSyncQueue = (__bridge id)dispatch_get_specific(kDispatchQueueSpecificKey);
    ...
    dispatch_sync(_queue, ^() {
        ...
        SRPFMDatabase *db = [self database];
        
        block(db);
        
        if ([db hasOpenResultSets]) {
            NSLog(@"Warning: there is at least one open result set around after performing [SRPFMDatabaseQueue inDatabase:]");
...
- (void)beginTransaction:(FMDBTransaction)transaction withBlock:(void (^)(SRPFMDatabase *db, BOOL *rollback))block {
    FMDBRetain(self);
    dispatch_sync(_queue, ^() { 
...
- (void)inTransaction:(void (^)(SRPFMDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionExclusive withBlock:block];
}

- (void)inDeferredTransaction:(void (^)(SRPFMDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionDeferred withBlock:block];
}

- (void)inExclusiveTransaction:(void (^)(SRPFMDatabase *db, BOOL *rollback))block {
    [self beginTransaction:FMDBTransactionExclusive withBlock:block];
}

- (void)inImmediateTransaction:(void (^)(SRPFMDatabase * _Nonnull, BOOL * _Nonnull))block {
    [self beginTransaction:FMDBTransactionImmediate withBlock:block];
}

- (NSError*)inSavePoint:(void (^)(SRPFMDatabase *db, BOOL *rollback))block {
