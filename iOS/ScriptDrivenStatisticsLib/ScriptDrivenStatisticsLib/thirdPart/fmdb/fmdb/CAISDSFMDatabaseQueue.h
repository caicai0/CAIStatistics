...
/** To perform queries and updates on multiple threads, you'll want to use `CAISDSFMDatabaseQueue`.

 Using a single instance of `<CAISDSFMDatabase>` from multiple threads at once is a bad idea.  It has always been OK to make a `<CAISDSFMDatabase>` object *per thread*.  Just don't share a single instance across threads, and definitely not across multiple threads at the same time.

 Instead, use `CAISDSFMDatabaseQueue`. Here's how to use it:

 First, make your queue.

    CAISDSFMDatabaseQueue *queue = [CAISDSFMDatabaseQueue databaseQueueWithPath:aPath];

 Then use it like so:

    [queue inDatabase:^(CAISDSFMDatabase *db) {
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];

        CAISDSFMResultSet *rs = [db executeQuery:@"select * from foo"];
        while ([rs next]) {
            //…
        }
    }];
 ...
 An easy way to wrap things up in a transaction can be done like this:
    ...
    [queue inTransaction:^(CAISDSFMDatabase *db, BOOL *rollback) {
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:1]];
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:2]];
        [db executeUpdate:@"INSERT INTO myTable VALUES (?)", [NSNumber numberWithInt:3]];
 ...
 `CAISDSFMDatabaseQueue` will run the blocks on a serialized queue (hence the name of the class).  So if you call `CAISDSFMDatabaseQueue`'s methods from multiple threads at the same time, they will be executed in the order they are received.  This way queries and updates won't step on each other's toes, and every one is happy.

 ### See also

 - `<CAISDSFMDatabase>`

 @warning Do not instantiate a single `<CAISDSFMDatabase>` object and use it across multiple threads. Use `CAISDSFMDatabaseQueue` instead.
 
 @warning The calls to `CAISDSFMDatabaseQueue`'s methods are blocking.  So even though you are passing along blocks, they will **not** be run on another thread.

 */

@interface CAISDSFMDatabaseQueue : NSObject
/** Path of database */
...
/** Create queue using path.
 ...
 @return The `CAISDSFMDatabaseQueue` object. `nil` on error.
 */
...
// If you need to nest, use CAISDSFMDatabase's startSavePointWithName:error: instead.
